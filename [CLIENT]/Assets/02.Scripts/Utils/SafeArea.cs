...
namespace Percent.Utils
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            if (Panel == null)
            {
                DebugLog.LogError("Cannot apply safe area - no RectTransform found on " + name);
                Destroy(gameObject);
            }
        ...
        void Refresh(bool _force = false)
        {
            Rect safeArea = GetSafeArea();

            if (_force || safeArea != LastSafeArea || Screen.width != LastScreenSize.x || Screen.height != LastScreenSize.y ||
        ...
        void ApplySafeArea(Rect r)
        {
            ...
            onSafeAreaTop?.Invoke(Screen.height - r.height - r.y);
            onSafeAreaBottom?.Invoke(r.y);
        ...
        [Space(20)]
        public SafeAreaEvent onSafeAreaTop;
        public SafeAreaEvent onSafeAreaBottom;
        // public SafeAreaEvent onSafeAreaLeft;
        // public SafeAreaEvent onSafeAreaRight;

        [System.Serializable]
        public class SafeAreaEvent : UnityEvent<float> { }


        private void OnUpdateSafeArea(params object[] _params)
        {
            Refresh(true);
        }
    }
